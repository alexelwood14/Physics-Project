...
def init_objects(window, resolution, border_inset, phy_consts, colours):
    ...
    #Image Buttons
    boids_button = pygame_ui.Image_Button(window, [resolution[0]/2 - resolution[0]/8, resolution[1]/3.5], (int(resolution[0]/6), int(resolution[1]/6)), "BOIDS SIMULATION", 28, "boids_sim", colours)
    particles_button = pygame_ui.Image_Button(window, [resolution[0]/2 + resolution[0]/8, resolution[1]/3.5], (int(resolution[0]/6), int(resolution[1]/6)), "PARTICLE SIMULATION", 28, "particle_sim", colours)
    compression_button = pygame_ui.Image_Button(window, [resolution[0]/2 - resolution[0]/8, resolution[1]/2], (int(resolution[0]/6), int(resolution[1]/6)), "COMPRESSION SIMULATION", 28, "compression_sim", colours)
    turning_button = pygame_ui.Image_Button(window, [resolution[0]/2 + resolution[0]/8, resolution[1]/2], (int(resolution[0]/6), int(resolution[1]/6)), "CAR SIMULATION", 28, "turning_sim", colours)
...
def main_menu(colours, window, resolution, border_inset, action, buttons, mouse_used):
    ...
    while action == "main":
        ...
        pygame_ui.draw_text(window, 'Alex Elwood | BETA (0.5.2)', [resolution[0] / 1.007, resolution[1]/1.02], int(resolution[1] / 56), colours["grey"], 'calibri', 'mr')
        pygame.display.update()
...
def main():
    ...
    while True:
        if action == "main":
            ...
            action, mouse_used = compression.simulation(window, clock, colours, resolution, phy_consts, action, mouse_used)
