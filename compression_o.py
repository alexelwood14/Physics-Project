...
class Compression_Ball:
    def __init__(self, window, border_inset, pos, phy_consts, mass, radius, node_count, colours):
        ...
        self.radius = radius
        self.mod_elast = 1000
        self.gas_force = 2 * math.pi * radius ** 2 * self.mod_elast * math.cos(
            (math.pi * (node_count - 2)) / 2 / node_count)
        ...
        for node in range(len(self.nodes)):
            if node != len(self.nodes) - 1:
                ...
                length = np.sqrt(vector.dot(vector))
                self.springs.append(
                    Ball_Spring(window, colours["blue"], length / 2, self.mod_elast * 100, self.nodes[node],
                                self.nodes[node + 1]))
    ...
    def resolve_forces(self, resolution, border_inset):
        for node in self.nodes:
            node.resolve_forces(self.gas_force, self.pos, self.area, resolution, border_inset, self.mass,
                                self.phy_consts)
