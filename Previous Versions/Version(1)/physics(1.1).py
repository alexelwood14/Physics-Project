...
def init_objects(window, resolution, border_inset, phy_consts, colours):
    square = objects.Square(window, border_inset, phy_consts, 10, 30, colours["red"])
    sphere = objects.Sphere(window, border_inset, phy_consts, 10, 15, colours["light blue"])
...
def simulation(window, clock, colours, resolution, border_inset, dyn_objects, sta_objects, sliders, phy_consts):
    ...
    dyn_objects["sphere"].set_pos([resolution[0]/4, resolution[1]/2])
    dyn_objects["sphere"].set_vel([-300.0, -1000.0])
    ...
    while True:
        ...
        if display_debug:
            ...
            draw_text(window, "gravitational acceleration: {}".format(phy_consts["g"]), text_size,
                      [resolution[0]/32, resolution[1]/14 + line_spacing], colours["white"])
            draw_text(window, "air density: {}".format(phy_consts["air_den"]), text_size,
                      [resolution[0]/32, resolution[1]/14 + line_spacing * 2], colours["white"])
            draw_text(window, "coefficient of restitution: {}".format(phy_consts["co_rest"]), text_size,
                      [resolution[0]/32, resolution[1]/14 + line_spacing * 3], colours["white"])
            draw_text(window, "coefficient of friction: {}".format(phy_consts["co_fric"]), text_size,
                      [resolution[0]/32, resolution[1]/14 + line_spacing * 4], colours["white"])
            draw_text(window, "throw strength: {}".format(phy_consts["throw_strength"]), text_size,
