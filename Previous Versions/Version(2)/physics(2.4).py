...
def init_objects(window, resolution, border_inset, phy_consts, colours):
    ...
    rand_colour_button = pygame_ui.Single_Button(window, [800, resolution[1] - 100],
                                          resolution[0]/8, 40, "RANDOMISE", colours["light_grey"], colours["grey"])

    rand_radius_button = pygame_ui.Single_Button(window, [1200, resolution[1] - 100],
                                          resolution[0]/8, 40, "RANDOMISE", colours["light_grey"], colours["grey"])

    rand_mass_button = pygame_ui.Single_Button(window, [1600, resolution[1] - 100],
                                          resolution[0]/8, 40, "RANDOMISE", colours["light_grey"], colours["grey"])

    #Initiate Ball Creators
    add_creator = pygame_ui.Add_Creator(window, [resolution[0] /1.55, 80] , colours)

    #Initiate scroll bar
    scroll_bar = pygame_ui.Scroll_Bar(window, [resolution[0] - 45, resolution[1] / 2], colours["grey"])
...
def simulation(window, clock, colours, resolution, border_inset, dyn_objects, sta_objects, sliders, phy_consts, action):
    ...
    for obj in dyn_objects:
        obj.set_pos([random.uniform(200,1500), random.uniform(200,800)])
        obj.set_vel([random.uniform(-500,500), random.uniform(-500,500)])
...
def main_menu(colours, window, resolution, border_inset, action, buttons, mouse_used, phy_consts):
    ...
    incriment = 100
    max_balls = 25
    creators.append(pygame_ui.Ball_Creator(window, [resolution[0] /1.55, 80 + incriment * len(creators)], colours))
    buttons["add_creator"].set_pos([resolution[0] /1.55, 80 + incriment * (len(creators))])
    dyn_objects = []
    ...
    while action == "main":
        ...
        #Sets all ball attributes to a random value
            ...
            for creator in creators:
                ...
                creator.set_radius(random.randint(1, 100))
            mouse_used = True
        ...
        elif buttons["add_creator"].highlight(mouse_used) and len(creators) <= max_balls:
            mouse_used = True
            creators.append(pygame_ui.Ball_Creator(window, [resolution[0] /1.55, list_pos + incriment * len(creators)], colours))
            buttons["add_creator"].set_pos([resolution[0] /1.55, 80 + incriment * (len(creators))])
        ...
        if offscreen > 0 and offscreen <= 9:
            used_buttons["scroll_bar"].set_size(1000 / (offscreen + 1))
            list_pos = 80 - slider_pos * offscreen * incriment
        elif offscreen >= 10:
            used_buttons["scroll_bar"].set_size(1000 / 10)
            list_pos = 80 - slider_pos * offscreen * incriment
        else:
            used_buttons["scroll_bar"].set_size(1000)
            list_pos = 80
            
        for creator in range(len(creators)):
            creators[creator].set_pos([resolution[0] /1.55, list_pos + incriment * creator])
        ...
        for creator in range(len(creators)):
            if mouse_used:
                temp, delete = creators[creator].highlight(mouse_used)
            else:
                mouse_used, delete = creators[creator].highlight(mouse_used)
            if delete:
                del_creator = creator
            creators[creator].render()
        if len(creators) <= max_balls:
            buttons["add_creator"].render()

        #Render Overlays
        pygame.draw.rect(window, colours["light_grey"], [600, 935, 1260, 165])
        ...
        if del_creator != -1:
            ...
            buttons["add_creator"].set_pos([resolution[0] /1.55, 80 + incriment * (len(creators))])
            for creator in range(len(creators)):
                creators[creator].set_pos([resolution[0] /1.55, 80 + incriment * creator])
