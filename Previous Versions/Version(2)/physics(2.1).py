...
def init_objects(window, resolution, border_inset, phy_consts, colours):
    ...
    back_button = pygame_ui.Single_Button(window, [resolution[0] - 240, resolution[1] - 120],
                                          resolution[0]/6, "BACK", colours["light_grey"], colours["grey"])

    ball_button = pygame_ui.Single_Button(window, [resolution[0]/2, resolution[1]/2 + 240],
                                          resolution[0]/6, "BALLS SIM", colours["light_grey"], colours["grey"])
...
def main_menu(colours, window, resolution, border_inset, action, buttons, mouse_used):
    ...
    while action == "main":
        ...
        if used_buttons["ball"].highlight(mouse_used):
            mouse_used = True
            action = "ball_menu"
...
def ball_menu(colours, window, resolution, border_inset, action, buttons, mouse_used):
    used_buttons = {"back" : buttons["back"],
                    "start" : buttons["start"],}
    while action == "ball_menu":
        #Reset mouse usage
        if not pygame.mouse.get_pressed()[0]:
            mouse_used = False
        ...
        #Process buttons if pressed
        if used_buttons["start"].highlight(mouse_used):
            mouse_used = True
            action = "sim"
...
def main():
    ...
    while True:
        ...
        elif action == "ball_menu":
            action, mouse_used = ball_menu(colours, window, resolution, border_inset, action, buttons, mouse_used)
        elif action == "sim":
