...
def main_menu(colours, window, resolution, border_inset, action, buttons, mouse_used):
    while action == "main":
        ...
        pygame.draw.lines(window, colours["grey"], True, ((0,0), (resolution[0],0),
                                                         (resolution[0],resolution[1]), (0,resolution[1])), int(resolution[1]/54))
        pygame_ui.draw_text(window, 'PHYSICS SIMULATIONS', [resolution[0]/2, resolution[1]/10.8], int(resolution[1] / 10.5), colours["grey"], 'calibri', 'c')
        pygame.display.update()
...
def main():
    ...
    while True:
        if action == "main":
            action = main_menu(colours, window, resolution, border_inset, action, buttons, mouse_used)
