...
def menu(colours, window, resolution, border_inset, action, buttons, mouse_used, phy_consts):
    ...
    used_buttons["quit"].set_size(resolution[0]/5, resolution[0] / 30)
    ...
    used_buttons["settings"].set_size(resolution[0]/5, resolution[0] / 30)
    ...
    used_buttons["quit"].set_pos([resolution[1] / 3.6, start + incriment * 4])
    used_buttons["initiate"].set_pos([resolution[1] / 3.6, start + incriment * 2])
    used_buttons["rand_all"].set_pos([resolution[1] / 3.6, start + incriment])
    ...
    used_buttons["settings"].set_pos([resolution[1] / 3.6, start + incriment * 3])
...
class Dynamic_object():
    ...
    def validate_position(self, resolution):
        max_pos = 99999
        if self.pos[0] < -max_pos or self.pos[0] > resolution[0] + max_pos or self.pos[1] < -max_pos or self.pos[1] > resolution[1] + max_pos:
    ...
    def force_lines(self, colours, resolution):
        ...
        pygame.draw.line(self.window, colours["blue"], self.pos, [self.pos[0], self.pos[1] + self.phy_consts["g"] / resolution[0]*640], 3)
        if abs(self.vel[0]) > 8:
            pygame.draw.line(self.window, colours["yellow"], self.pos, [self.pos[0] + self.fric / resolution[0]*640, self.pos[1]], 3)
        if self.on_ground or self.ground_col:
            pygame.draw.line(self.window, colours["green"], self.pos, [self.pos[0], self.pos[1] - self.phy_consts["g"] / resolution[0]*640], 3)
